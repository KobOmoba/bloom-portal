...
async function doLogin(){
  ...
  const btn=$('l-btn');
  if(!sid||!email||!pwd){err.textContent='Fill in all fields.';err.style.display='block';return;}
  ...
  try{
    ...
    const user=(school.staff||[]).find(s=>s.email===email&&s.password===pwd);
    if(!user){err.textContent='Wrong email or password.';err.style.display='block';btn.textContent='▶ Login';btn.disabled=false;return;}
