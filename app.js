...
async function doLogin(){
  ...
  try{
    ...
    if(lc&&ls){
      ...
      let user=null;
      if(email){user=staff.find(s=>s.email===email&&s.password===pwd);}
      if(!user){user=staff.find(s=>s.password===pwd);}
      if(user){schoolId=sid;userRole=user.role;localStorage.setItem('p_auth',JSON.stringify({schoolId:sid,email:user.email,role:user.role}));
